...
function getSaleItemPayload(body) {
  ...
  return {
    ...
    date_sold: parseDate(body.date_sold, 'date_sold', errors, { required: true }),
    details: parseText(body.details, 'details', errors, { required: true }),
    errors,
    make: parseText(body.make, 'make', errors),
    model: parseText(body.model, 'model', errors),
    payment_type: parseText(body.payment_type, 'payment_type', errors, {
      required: true,
    }),
    price: parseNumber(body.price, 'price', errors, { min: 0, required: true }),
    type: parseText(body.type, 'type', errors, { required: true }),
  };
}
